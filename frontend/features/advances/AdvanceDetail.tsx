...
/**
 * AdvanceDetail — extracted detail component (summary, FSM, documents,
 * review timeline, state history).
 *
 * RF-046: extracted from the inline detail page so the page is a thin
 * adapter and the detail is testable in isolation.
 */
...
export function AdvanceDetail({ advance, advanceId, projectId, userId }: AdvanceDetailProps) {
  const router = useRouter();
  const deleteAdvance = useDeleteAdvance();
  const [deleteOpen, setDeleteOpen] = useState(false);

  const canEdit = canEditAdvance(advance);
  const canDelete = canDeleteAdvance(advance, userId);
