"""
RLS (Row-Level Security) policies for reports-scoped tables.

Implements tenant isolation for reports tables:
- reports_report (parent: direct institution_id)
- reports_reportapproval (child: subquery via report_id)

Design reference: openspec/changes/reports/design.md — RLS Policies
Pattern reference: apps/progress/migrations/0002_rls_policies.py
...
PARENT_TABLE = "reports_report"

# ── Child tables (no institution_id — reach via report_id FK) ───────────

CHILD_TABLES = [
    "reports_reportapproval",
]
...
# ── Child table policies (subquery via report_id) ──────────────────────
...
-- Policy: users see only rows linked to reports in their institution
DROP POLICY IF EXISTS tenant_isolation ON {table};
CREATE POLICY tenant_isolation ON {table}
    USING (report_id IN (
        SELECT id FROM reports_report
