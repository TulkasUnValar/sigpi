"""
RLS policy tests for calls app — STRICT TDD (RED phase).

Tests verify the existence and structure of migration 0002_rls_policies.py:
- Migration class exists in the calls migrations module
- Contains a RunPython operation with reverse_code
- Depends on 0001_initial
- SQL targets all 4 calls tables
- tenant_isolation + superadmin_bypass policies present for each table
- Parent table (calls_call) uses direct institution_id
- Child tables use subquery via call_id
- PostgreSQL-only guard (no-op on SQLite)

Spec reference:  openspec/changes/calls/spec.md — Security Requirements
Design reference: openspec/changes/calls/design.md — RLS Policies
Pattern reference: projects/migrations/0002_rls_policies.py
...
def _get_migration():
    """Load the calls 0002 migration from the migration loader."""
    loader = MigrationLoader(connection)
    key = ("calls", "0002_rls_policies")
...
class TestRLSMigrationExists:
    ...
    def test_migration_0002_on_disk(self, db):
        ...
        assert migration is not None, (
            ...
            "Expected at: apps/calls/migrations/0002_rls_policies.py"
        )
    ...
    def test_depends_on_0001(self, db):
        ...
        assert ("calls", "0001_initial") in deps, (
            f"Expected dependency on calls.0001_initial, got: {deps}"
        )
...
class TestRLSPolicySQL:
    ...
    def test_parent_table_uses_direct_institution_id(self, db):
        """Call table must filter by institution_id directly (has the column)."""
        migration = _get_migration()
        assert migration is not None, "Migration 0002 missing"
        sql = self._get_all_sql()
...
class TestRLSPostgresGuard:
    ...
    def test_has_is_postgresql_check(self, db):
        ...
        assert "vendor" in source.lower() or "postgresql" in source.lower(), (
            ...
            "(e.g., SQLite in tests). Follow projects/0002 pattern."
        )
