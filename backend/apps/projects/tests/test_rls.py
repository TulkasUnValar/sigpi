"""
RLS policy tests for projects app — STRICT TDD (RED phase).

Tests verify the existence and structure of migration 0002_rls_policies.py:
- Migration class exists in the projects migrations module
- Contains a RunPython operation with reverse_code
- Depends on 0001_initial
- SQL targets all 5 project tables
- tenant_isolation + superadmin_bypass policies present for each table
- Parent table (projects_project) uses direct institution_id
- Child tables use subquery via project_id
- PostgreSQL-only guard (no-op on SQLite)

Spec reference:  openspec/changes/projects/spec.md — Security Requirements
Design reference: openspec/changes/projects/design.md — RLS Policies
Pattern reference: researchers/migrations/0002_rls_policies.py
...
def _get_migration():
    """Load the projects 0002 migration from the migration loader."""
    loader = MigrationLoader(connection)
    key = ("projects", "0002_rls_policies")
...
class TestRLSMigrationExists:
    ...
    def test_migration_0002_on_disk(self, db):
        ...
        assert migration is not None, (
            ...
            "Expected at: apps/projects/migrations/0002_rls_policies.py"
        )
    ...
    def test_depends_on_0001(self, db):
        ...
        assert ("projects", "0001_initial") in deps, (
            f"Expected dependency on projects.0001_initial, "
            f"got: {deps}"
        )
...
class TestRLSPolicySQL:
    ...
    def test_parent_table_uses_direct_institution_id(self, db):
        """Project table must filter by institution_id directly (has the column)."""
        migration = _get_migration()
        assert migration is not None, "Migration 0002 missing"
        sql = self._get_all_sql()
        assert "institution_id = current_setting" in sql, (
            "projects_project must use direct institution_id filter. "
...
class TestRLSPostgresGuard:
    ...
    def test_has_is_postgresql_check(self, db):
        ...
        assert "vendor" in source.lower() or "postgresql" in source.lower(), (
            ...
            "(e.g., SQLite in tests). Follow researchers/0002 pattern."
        )
