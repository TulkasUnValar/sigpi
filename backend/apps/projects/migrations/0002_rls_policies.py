"""
RLS (Row-Level Security) policies for project-scoped tables.

Implements tenant isolation for all 5 project tables:
- projects_project (parent: direct institution_id)
- projects_projectmember (child: subquery via project_id)
- projects_projectdocument (child: subquery via project_id)
- projects_projectobservation (child: subquery via project_id)
- projects_projectstatelog (child: subquery via project_id)

Design reference: openspec/changes/projects/design.md — RLS Policies
Pattern reference: apps/researchers/migrations/0002_rls_policies.py
...
PARENT_TABLE = "projects_project"

# ── Child tables (no institution_id — reach via project_id FK) ──────────
...
# ── Child table policies (subquery via project_id) ──────────────────────
...
-- Policy: users see only rows linked to projects in their institution
